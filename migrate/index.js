...
async function opinionGroup(org, maria) {
    ...
    for (const row of rows) {
        await post`
          ...
          values
            (${row.id + org.offset}, ${row.name}, null, ${row.sort_index || 0}, ${org.name.toUpperCase()})
        `
    }
}
...
async function opinion(org, maria) {
    ...
    for (const row of rows) {
        await post`
          ...
          values
            (${row.id + org.offset}, ${row.title}, ${row.about}, ${row.document_link}, ${row.opinion_group + org.offset}, ${row.published_at}, ${row.published}, ${org.name.toUpperCase()})
        `
    }
}
...
async function news(org, maria) {
    ...
    for (const row of rows) {
        await post`
          ...
          values
            (${row.id + org.offset}, ${row.title}, ${row.content}, ${row.synopsis}, ${row.published}, ${row.published_at}, ${row.author}, ${row.banner_image}, ${row.banner_image_alt}, ${org.name.toUpperCase()})
        `
    }
}
...
async function questionCategory(org, maria) {
    ...
    for (const row of rows) {
        ...
        await post`
          ...
          values
            (${parseInt(row.id) + org.offset}, ${row.title}, ${row.slug}, ${org.name.toUpperCase()}, ${row.description}, ${row.sort_index})
        `
    }
}
...
async function question(org, maria) {
    ...
    for (const row of rows) {
        await post`
          ...
          values
            (${row.id + org.offset}, ${row.question}, ${row.answer}, ${org.name.toUpperCase()}, ${parseInt(row.question_category_id) + org.offset}, ${row.sort_index})
        `
    }
}
