            ...
            <div class="space-y-0">
                ...
                <div class="space-y-6">
                    {#each group.opinions as opinion}
                        <div class="h-[3px] first:bg-transparent bg-black/20"></div>
                        <div>
                            <a href="https://gentsestudentenraad.be/static/persistent/files/{opinion.document_link}" class="space-y-1">
                                <p class="text-primary decoration-2 font-bold text-xl">{opinion.title}</p>
                                {#if opinion.about}
                                    <p>{opinion.about}</p>
                                {/if}
                                <p class="font-bold text-sm opacity-75">{opinion.published_at.toLocaleDateString()}</p>
                            </a>
                        </div>
                    {/each}
                </div>
